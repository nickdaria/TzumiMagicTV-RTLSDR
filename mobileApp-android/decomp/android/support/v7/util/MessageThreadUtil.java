...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02831();
            ...
            class C02831 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C02842.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C02842.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C02842.this.mQueue.removeMessages(2);
                                    C02842.this.mQueue.removeMessages(3);
                        ...
                        C02842.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02811();
            final MessageQueue mQueue = new MessageQueue();
            ...
            class C02811 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem next = C02821.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
